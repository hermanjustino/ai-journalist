  ...
  async fetchFromScholar(options = {}) {
    ...
    try {
      ...
      return articles.map((article: any) => {
        ...
        let abstract = article.abstract || article.snippet || '';
        
        // Add AAVE-related content to some mock articles for demo purposes
        if (article.id && article.id.includes('scholar-mock') && 
            !abstract.includes('AAVE') && Math.random() > 0.5) {
          abstract += ' The study identified patterns like "he going" and "they be working" in classroom discourse.';
        }
